    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        if command -v apt &> /dev/null; then
            ...
            if command -v dnf &> /dev/null; then
                sudo dnf install -y qemu-kvm qemu-img libvirt wget cloud-utils
            else
                sudo yum install -y qemu-kvm qemu-img libvirt wget cloud-utils
            fi
            
        elif command -v pacman &> /dev/null; then
            # Arch Linux
            sudo pacman -S --noconfirm qemu-full wget cloud-init
            
        elif command -v zypper &> /dev/null; then
            # openSUSE
            sudo zypper install -y qemu-kvm qemu-tools wget cloud-init
            
        else
            print_status "ERROR" "Could not detect package manager. Please install manually:"
            print_status "INFO" "On Ubuntu/Debian: sudo apt install qemu-system qemu-utils cloud-image-utils wget"
            print_status "INFO" "On RHEL/CentOS: sudo yum install qemu-kvm qemu-img libvirt wget cloud-utils"
            exit 1
        fi
...
check_kvm_support() {
    ...
    if grep -q -E 'vmx|svm' /proc/cpuinfo; then
        print_status "SUCCESS" "CPU virtualization support detected"
    else
        print_status "ERROR" "CPU does not support virtualization (VT-x/AMD-V)"
        print_status "INFO" "Please enable virtualization in your BIOS/UEFI settings"
        exit 1
    fi
    
    # Check if KVM module is loaded
    if ! lsmod | grep -q kvm; then
        print_status "WARN" "KVM kernel module is not loaded"
        
        # Try to load KVM module
        print_status "INFO" "Attempting to load KVM module..."
        
        # Check if running as root
        if [ "$EUID" -eq 0 ]; then
            # Try to load appropriate KVM module
            if grep -q -E 'vmx' /proc/cpuinfo; then
                modprobe kvm_intel 2>/dev/null || true
            elif grep -q -E 'svm' /proc/cpuinfo; then
                modprobe kvm_amd 2>/dev/null || true
            fi
            modprobe kvm 2>/dev/null || true
        else
            print_status "INFO" "Trying to load KVM with sudo..."
        ...
        # Check again
        if lsmod | grep -q kvm; then
            print_status "SUCCESS" "KVM module loaded successfully"
        else
            print_status "WARN" "Could not load KVM module automatically"
            print_status "INFO" "You may need to:"
            print_status "INFO" "1. Enable KVM in BIOS/UEFI settings"
            print_status "INFO" "2. Load KVM module manually: sudo modprobe kvm"
            print_status "INFO" "3. Check if virtualization is disabled in Windows Features (for WSL)"
            
            # Ask if user wants to continue without KVM (slower)
            read -p "$(print_status "INPUT" "Continue without KVM (performance will be slower)? (y/N): ")" -n 1 -r
            echo
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
                exit 1
            fi
    ...
    if [ ! -r /dev/kvm ] || [ ! -w /dev/kvm ]; then
        print_status "WARN" "User does not have access to /dev/kvm"
        
        # Try to fix permissions
        if [ "$EUID" -eq 0 ]; then
            chmod 666 /dev/kvm 2>/dev/null || true
        else
            print_status "INFO" "Trying to fix /dev/kvm permissions with sudo..."
        ...
        if command -v usermod &> /dev/null; then
            if [ "$EUID" -eq 0 ]; then
                usermod -a -G kvm "$(whoami)" 2>/dev/null || true
            else
                sudo usermod -a -G kvm "$(whoami)" 2>/dev/null || true
            fi
            print_status "INFO" "You may need to logout and login again for group changes to take effect"
        fi
    fi
    
    # Final KVM check
    if [ -c /dev/kvm ] && [ -r /dev/kvm ] && [ -w /dev/kvm ]; then
        print_status "SUCCESS" "KVM is properly configured"
    else
        print_status "WARN" "KVM is not fully configured. VMs will run in software mode (slower)."
        print_status "INFO" "To fix:"
        print_status "INFO" "1. sudo usermod -a -G kvm $USER"
        print_status "INFO" "2. Logout and login again"
        print_status "INFO" "3. sudo chmod 666 /dev/kvm"
    fi
}
